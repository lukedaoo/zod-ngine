...
static void reset(void) {
    ...
    config_priv_seed_preset(&g_ctx.config);
}
...
MU_TEST(test_preset_int_defaults) {
    ...
    mu_assert_int_eq(0, zngine_config_get_int("log.level", -1));
}
...
MU_TEST(test_fallback_on_missing) {
    reset();
    mu_assert_int_eq(42, zngine_config_get_int("no.such.key", 42));
    mu_check(zngine_config_get_float("no.such.key", 1.5f) == 1.5f);
    mu_check(zngine_config_get_bool("no.such.key", true) == true);
    mu_assert_string_eq("fb", zngine_config_get_string("no.such.key", "fb"));
}

MU_TEST(test_set_get_roundtrip) {
    reset();
    mu_check(zngine_config_set_int("window.width", 1920));
    mu_assert_int_eq(1920, zngine_config_get_int("window.width", 0));

    mu_check(zngine_config_set_bool("window.vsync", false));
    mu_check(zngine_config_get_bool("window.vsync", true) == false);

    mu_check(zngine_config_set_string("window.title", "custom"));
    mu_assert_string_eq("custom", zngine_config_get_string("window.title", ""));
}
...
MU_TEST(test_engine_config_valid_without_cvars) {
    ...
    mu_assert_int_eq(99, zngine_config_get_int("window.width", 99));
    mu_check(zngine_config_get_float("window.height", 1.0f) == 1.0f);
    mu_check(zngine_config_get_bool("window.vsync", false) == false);
    mu_assert_string_eq("fb", zngine_config_get_string("window.title", "fb"));
}

MU_TEST(test_adjust_config_converts_string_debug_to_int) {
    reset();
    cvar_set_string(&g_ctx.config.cvars, "log.level", "debug");
    mu_assert_int_eq(-1, zngine_config_get_int("log.level", -1));
    mu_check(config_priv_adjust(&g_ctx.config));
    mu_assert_int_eq(LOG_DEBUG, zngine_config_get_int("log.level", -1));
}

MU_TEST(test_adjust_config_leaves_int_unchanged) {
    reset();
    mu_check(config_priv_adjust(&g_ctx.config));
    mu_assert_int_eq(LOG_TRACE, zngine_config_get_int("log.level", -1));
}
...
MU_TEST(test_adjust_config_all_string_levels) {
    ...
    for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        mu_check(config_priv_adjust(&g_ctx.config));
        mu_assert_int_eq(cases[i].v, zngine_config_get_int("log.level", -1));
    }
}

MU_TEST(test_adjust_config_null_returns_false) { mu_check(!config_priv_adjust(NULL)); }
...
MU_TEST(test_reload_no_watcher_returns_false) {
    ...
    mu_check(!config_priv_reload_from_file(&g_ctx.config));
}
...
MU_TEST(test_reload_success_updates_config) {
    ...
    mu_check(config_priv_reload_from_file(&g_ctx.config));
    mu_assert_int_eq(1280, zngine_config_get_int("window.width", 0));
    mu_assert_int_eq(720, zngine_config_get_int("window.height", 0));
    mu_assert_int_eq(60, zngine_config_get_int("engine.target_fps", 0));
...
MU_TEST(test_reload_fail_preserves_config) {
    reset();
    zngine_config_set_int("window.width", 1920);
