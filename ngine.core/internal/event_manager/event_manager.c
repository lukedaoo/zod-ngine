...
void event_manager_priv_init(event_manager *mgr) {
    event_table_init(&mgr->dispatcher);
    event_manager_priv_subscribe_sys_events(mgr);
}
void event_manager_priv_destroy(event_manager *mgr) { event_table_destroy(&mgr->dispatcher); }
...
event_handle event_manager_priv_subscribe(event_manager       *mgr,
    ...
    return event_table_subscribe(&mgr->dispatcher, category, event_id, callback, userdata,
                                 destroy_fn);
}

bool event_manager_priv_unsubscribe(event_manager *mgr, const event_handle handle) {
    if (!mgr) return false;
    return event_table_unsubscribe(&mgr->dispatcher, handle);
}

bool event_manager_priv_unsubscribe_by_event_identifier(event_manager       *mgr,
                                                        const event_category category,
                                                        const int            event_id) {
    return event_table_unsubscribe_by_event_identifier(&mgr->dispatcher, category, event_id);
}

void event_manager_priv_publish(event_manager *mgr, event_context *ctx) {
    event_table_publish(&mgr->dispatcher, ctx);
}

void event_manager_priv_emit(event_manager *mgr, const event_category category,
                             const int event_id, void *payload,
                             const size_t payload_size) {
    event_table_emit(&mgr->dispatcher, category, event_id, payload, payload_size);
}

void event_manager_priv_subscribe_sys_events(event_manager *mgr) {
    event_table_subscribe(&mgr->dispatcher, EVENT_CATEGORY_SYSTEM, SYS_EVENT_ON_RESIZE_WINDOW,
                          sys_event_priv_on_resize_window, NULL, NULL);
    event_table_subscribe(&mgr->dispatcher, EVENT_CATEGORY_SYSTEM,
