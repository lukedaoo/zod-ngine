...
int main() {
    ...
    const char *frag =
         ...
         "out vec4 frag_color;\n"
         "void main() {\n"
         "    if (is_point) frag_color = vec4(v_out_color, 1.0);\n"
         "    else frag_color = vec4(1.0, 1.0, 1.0, 1.0);\n"
         "}\n";
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        glBindVertexArray(vao);
        GLint is_point = glGetUniformLocation(shader_program, "is_point");
        glUniform1i(is_point, 0);  // false
        glDrawArrays(GL_LINES, 0, 2);

        glUniform1i(is_point, 1);  // true
        glPointSize(20.0f);
