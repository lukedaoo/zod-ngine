...
MU_TEST(test_subscribe_multiple_listeners_same_event) {
    ...
    mu_check(
         event_table_subscribe(&table, EVENT_TAG_SYSTEM_INPUT, 1, spy_callback, &rec_b));
...
MU_TEST(test_subscribe_null_callback_returns_false) {
    ...
    mu_check(!event_table_subscribe(&table, EVENT_TAG_SYSTEM_INPUT, 1, NULL, NULL));
...
MU_TEST(test_publish_calls_matching_listener) {
    ...
    event_context ctx     = {
         .identifier   = {.category = EVENT_TAG_SYSTEM_INPUT, .event_id = 1},
         .payload      = &payload,
         .payload_size = (int)sizeof(payload)};
    event_table_publish(&table, &ctx);
    ...
    mu_assert_int_eq((int)sizeof(payload), rec.last_payload_size);
...
MU_TEST(test_integration_subscribe_publish_fires) {
    ...
    spy_record rec = spy_record_new();
    event_table_subscribe(&table, EVENT_TAG_APPLICATION, 7, spy_callback, &rec);
...
MU_TEST(test_integration_two_listeners_fire_in_order) {
    ...
    event_table_subscribe(&table, EVENT_TAG_APPLICATION, 7, spy_callback, &rec_a);
    event_table_subscribe(&table, EVENT_TAG_APPLICATION, 7, spy_callback, &rec_b);
